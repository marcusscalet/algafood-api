...
public interface GroupControllerOpenApi {
	...
	public GroupDTO find(@ApiParam(value = "ID do grupo", example = "1", required = true) Long groupId);

	@ApiResponses({ @ApiResponse(code = 201, message = "Grupo cadastrado")})
	@ApiOperation("Cria um novo grupo")
	public GroupDTO add(@ApiParam(name = "body", value = "Representação de um grupo", required = true) GroupInput groupInput);

	@ApiOperation("Atualiza grupo por ID")
	@ApiResponses({ @ApiResponse(code = 200, message = "Grupo atualizado"),
			@ApiResponse(code = 404, message = "Grupo não encontrado", response = Problem.class) })
	public GroupDTO update(@ApiParam(value = "ID do grupo", example = "1", required = true) Long groupId,
	...
	@ApiResponses({ @ApiResponse(code = 204, message = "Grupo removido"),
			@ApiResponse(code = 404, message = "Grupo não encontrado", response = Problem.class) })
	public void remove(@ApiParam(value = "ID do grupo", example = "1", required = true) Long groupId);
